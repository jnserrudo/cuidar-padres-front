  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto w-full max-w-5xl px-6 pb-20" id="inicio">
        <section className="py-14 text-center">
          <div className="mx-auto max-w-3xl space-y-6">
            <h1 className="font-serif text-4xl leading-tight text-slate-800 sm:text-5xl">
              Acompañamos a quienes cuidan, escuchan y sostienen.
            </h1>
            <p className="text-lg text-slate-600">
              Somos una comunidad para familiares que cuidan a personas mayores.
              Compartimos experiencias, información y contactos para que nadie atraviese el proceso en soledad.
            </p>
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              <Link to="/solicitar" className="inline-flex items-center justify-center gap-2 rounded-full bg-[color:var(--color-terracotta)] px-7 py-3 text-sm font-semibold text-white shadow-sm transition hover:brightness-95 focus-visible:outline focus-visible:outline-2 focus-visible:outline-[color:var(--color-terracotta)]">
                Solicitar ingreso <svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14" /><path d="M13 6l6 6-6 6" /></svg>
              </Link>
        ...
        <section className="py-8" id="ofrecemos">
          <div className="text-center">
            <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[color:var(--color-rose)] text-[color:var(--color-terracotta)]">
              <svg aria-hidden="true" viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l-2-2a2 2 0 0 0-3 0L2 11" /><path d="M15 11l2-2a2 2 0 0 1 3 0l2 2" /><path d="M7 13l3 3a2 2 0 0 0 3 0l1-1" /><path d="M17 13l-3 3a2 2 0 0 1-3 0l-1-1" /></svg>
            </span>
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {offers.map((offer, index) => (
              <div key={offer.title} className={`rounded-2xl border border-black/5 bg-white p-6 shadow-sm ${index === offers.length - 1 ? "sm:col-span-2 sm:justify-self-center sm:max-w-md sm:w-full lg:col-span-1 lg:col-start-2" : ""}`}>
                ...
                <p className="mt-4 text-base font-semibold text-slate-800">{offer.title}</p>
        ...
        <section className="py-8" id="reglas">
          <div className="text-center">
            <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-[color:var(--color-rose)] text-[color:var(--color-terracotta)]">
              <svg aria-hidden="true" viewBox="0 0 24 24" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l7 4v5c0 5-3.5 8.5-7 9-3.5-.5-7-4-7-9V7z" /><path d="M9 12l2 2 4-4" /></svg>
            </span>
            <h2 className="mt-4 text-2xl font-semibold text-slate-800">Reglas del grupo</h2>
            <p className="mt-2 text-sm text-slate-500">Cuidamos el respeto y la privacidad.</p>
          </div>
        ...
        <section className="py-10">
          <div className="rounded-3xl border border-black/5 bg-white px-6 py-10 text-center shadow-sm sm:px-8">
            ...
            <Link to="/solicitar" className="mt-6 inline-flex items-center justify-center gap-2 rounded-full bg-[color:var(--color-terracotta)] px-7 py-3 text-sm font-semibold text-white shadow-sm transition hover:brightness-95">Solicitar ingreso</Link>
          </div>
